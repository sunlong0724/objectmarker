...
char* outputname = "info.txt";
char* output_dir = "cropped";
char* done_dir = "done";
char* roi_dir = "roi";
char* raw_image_ext = ".bmp";
...
void printHelp() {
	printf("/nObject Marker: \n/ta tool to annotate the locations of objects in an image\n");
	printf("/tGunawan Herman, April 2006\n");
	printf("/tAdapted from ObjectMarker.cpp by A.Florian\n");
	printf("\n");
	printf("/tThis program searches for input images in dir '%s'\n", input_dir);
	printf("/tLocations of objects will be printed out to file '%s'\n", outputname);
	printf("\n");
	printf("------------------------------------------------------------\n");
	printf("|  btn  |               function                           |\n");
	printf("|-------|--------------------------------------------------|\n");
...
void redrawImage() {
	...
	if (roi_x0 > 0) {
		cvRectangle(image2, cvPoint((int)roi_x0, (int)roi_y0), cvPoint((int)roi_x1, (int)roi_y1), CV_RGB(255, 0, 0), 2);
		//cvLine(image2,cvPoint(0,roi_y1),cvPoint(image2->width,roi_y1),CV_RGB(255,255,0),1,8,0);

	}
	if (roi_x0 > 0) {
		//cvRectangle(image2, cvPoint((int)roi_x0,(int)roi_y0), cvPoint((int)roi_x1,(int)roi_y1), CV_RGB(255,0,0),1);
		cvLine(image2, cvPoint(roi_x1, 0), cvPoint(roi_x1, image2->height), CV_RGB(255, 255, 0), 2, 8, 0);
		cvLine(image2, cvPoint(0, roi_y1), cvPoint(image2->width, roi_y1), CV_RGB(255, 255, 0), 2, 8, 0);
...
void on_mouse(int event, int x, int y, int flag, void*) {
	...
	if (event == CV_EVENT_MOUSEMOVE && flag == 33) {
		...
		if (flag == CV_EVENT_FLAG_LBUTTON) {
			//cvLine(image,cvPoint(0,y),cvPoint(640,y),CV_RGB(255,255,0),1,8,0);
			redrawImage();
		}
		cornerImage();
	}
...
void saveObjects(FILE* output, const char* imagename) {
	int numOfRect = (int)(objects.size());
	IplImage* tmp_image = cvCloneImage(image);
	if (numOfRect) {
		...
		for (int i = 0; i < (int)(objects.size()); i++) {
			...
			cvSetImageROI(tmp_image, cvRect(rectangle.x, rectangle.y, rectangle.width, rectangle.height));
			char file_name[1024] = {0};
			//string original_name(imagename);
			//string tmp = original_name.substr(0, original_name.rfind('.'));
			string name(imagename);
			string tmp = name.substr(strlen(input_dir), name.length());
			string output_name(roi_dir);
			output_name.append(tmp);
			//tmp = output_name.substr(0, output_name.rfind('.'));

			snprintf(file_name, sizeof file_name, "%s_%d_%d_%d_%d_%d.bmp", output_name.c_str(),i,rectangle.x, rectangle.y, (int)rectangle.width, (int)rectangle.height);
			//cout << file_name << endl;
			cvSaveImage(file_name, tmp_image);
	...
	string name(imagename);
	string tmp = name.substr(strlen(input_dir), name.length());
	string output_name(done_dir);
	output_name.append(tmp);
	
	//cout << output_name << endl;
	cvSaveImage(output_name.c_str(), image);

	char dir[1024];
	char full_name[1024];
	GetCurrentDirectoryA(sizeof dir, dir);
	snprintf(full_name, sizeof full_name, "%s\\%s", dir, imagename);
	cout << full_name << endl;
	DeleteFileA(imagename);
}
...
int cropImages(const char* datafile) {
	...
	while (true) {
		...
		for (int i = 0; i < nrects; i++) {
			...
			tmpimage->roi = &roi;
			sprintf(crop_output, "%s/%05d.bmp", output_dir, index++);
...
int _tmain(int argc, _TCHAR* argv[]) {
	if (argc > 1) {
		...
		if (n > 0) {
			printf(": %s/%05d.bmp - %s/%05d.bmp", output_dir, 1, output_dir, n);
		}
		printf("\n");
		exit(0);
	}
	...
	if ((hFile = (long)_findfirst(pattern, &bmp_file)) == -1L) {
		...
		do {
			...
			do {
				...
				case Key_d:
					cont = true;
					if (!objects.empty()) {//added sunruilong
						objects.pop_back();
						redrawImage();
					}
					break;
